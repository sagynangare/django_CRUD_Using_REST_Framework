...
class Notes(generics.GenericAPIView):
	...
	def post(self,request):#Create/POST
		...
		if serializer.is_valid():
			...
			return Response({
				"status":'success',
				'data':{'note':serializer.data}}, status=status.HTTP_201_CREATED)
		else:
			return Response({
				'status':'fail',
...
class NoteDetail(generics.GenericAPIView):
	...
	def get(self,request,pk):
		...
		if note== None:
			return Response({
				"status":"fail",
				"message":f"Note with ID:{pk} not found"}, status=status.HTTP_404_NOT_FOUND)
		serializer=self.serializer_class(note)
		return Response({
				"status":"success",
				'data':{'note':serializer.data}})

	def patch(self,request,pk):#Update/Patch
		note=self.get_note(pk=pk)
		if note==None:
			return Response({
				"status":"fail",
		...
		if serializer.is_valid():
			serializer.validated_data['uploadedAt']=datetime.now()
			serializer.save()
			return Response({
				"status":"success",
				'data':{'note':serializer.data}})
